...
app.get('/make-server-e0c14ace/brand-settings', async (c) => {
  try {
    const settings = await kv.get('brand-settings');
    return c.json({ success: true, settings });
  } catch (error) {
    console.log('Error fetching brand settings:', error);
    return c.json({ success: false, error: 'Failed to fetch brand settings' }, 500);
  }
});

app.post('/make-server-e0c14ace/brand-settings', async (c) => {
  try {
    const settings = await c.req.json();
    await kv.set('brand-settings', settings);
    return c.json({ success: true, settings });
  } catch (error) {
    console.log('Error saving brand settings:', error);
    return c.json({ success: false, error: 'Failed to save brand settings' }, 500);
  }
});

// Logo upload endpoint
app.post('/make-server-e0c14ace/upload-logo', async (c) => {
  try {
    ...
    const bucketName = 'make-e0c14ace-logos';
    ...
    // Create signed URL for accessing the logo
    ...
    if (urlError) {
      ...
      return c.json({ success: false, error: 'Failed to create access URL' }, 500);
    }
...
app.delete('/make-server-e0c14ace/logo/:fileName', async (c) => {
  try {
    ...
    if (error) {
      ...
      return c.json({ success: false, error: 'Failed to delete logo' }, 500);
    }
